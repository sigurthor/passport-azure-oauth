...
Strategy.prototype.authenticate = function(req, options) {
  console.log('hostheaders', req.headers);
  options = options || {};
  var self = this;
  ...
  if (!this._clientId)
    throw new Error('AzureOAuthStrategy requires a clientId.');
  if (!this._clientSecret)
    throw new Error('AzureOAuthStrategy requires a clientSecret.');
  if (!this._tenantId)
    throw new Error('AzureOAuthStrategy requires a tenant id.');
  ...
  if (typeof this._refreshToken != 'undefined') {
    ...
    this._oauth2.getOAuthAccessToken(
      this._refreshToken,
      {
        ...
        redirect_uri: redirectURL.replace('{host}', req.headers.host)
      },
    ...
    if (this.redirectURL) {
      requestParams.redirect_uri = this.redirectURL.replace('{host}', req.headers.host);
    }
    ...
    if (this.redirectURL) {
      params['redirect_uri'] = this.redirectURL.replace('{host}', req.headers.host);
    }
...
Strategy.prototype.authorizationParams = function(options) {
  ...
  for (var attr in options) {
    if (attr != 'redirectURL' && attr != 'clientId' && attr != 'clientSecret' && attr != 'resource' && attr != 'tenantId') {
